...
RSpec.describe "イベント", type: :system do
  describe "一覧表示" do
    before do
      create(:event, kind: "Symposium")
      create(:event, kind: "Colloquium")
    end
  ...
  describe "詳細表示" do
    let!(:event) { create(:event, kind: "Colloquium", date: "2022-07-21".to_date, title: "Investigating the Battle of Hastings", speaker: "Sarah Croix", host: "Jim Bradbury", published: false) }
    ...
    context "一覧ページから詳細ページに移動したとき" do
      it "詳細が表示される" do
        ...
        expect(page).to have_content "Kind: Colloquium"
  ...
  describe "作成" do
    ...
    context "入力内容が無効なとき" do
      it "エラーメッセージが表示される" do
        ...
        expect(page).to have_content "You must enter an event kind"
  ...
  describe "更新" do
    let!(:event) { create(:event, kind: "Symposium", date: "2022-03-30".to_date) }
    ...
    context "入力内容が有効なとき" do
      it "トーストメッセージが表示される、更新したイベントが表示される" do
        ...
        expect(page).to have_content "Kind: Symposium"
        expect(page).to have_current_path "/events/#{event.id}"

        click_link "Edit"

        expect(page).to have_field "Kind:", with: "Symposium"
  ...
  describe "削除" do
    let!(:event) { create(:event, kind: "Symposium", date: "2022-03-01".to_date) }
